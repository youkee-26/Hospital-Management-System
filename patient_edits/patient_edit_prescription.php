...
<html lang="en">
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div class="container-fluid">
          <a class="navbar-brand" href="#">Your Prescriptions</a>
  ...
  <div class="mb-10">
    <label for="PRESCRIPTION" class="form-label">Search prescriptions by date</label>
    <input type="PRESCRIPTION" class="form-control" id="appointmentserial" name="search_year">
...
<table class="table">
  <thead>
    <tr>
      ...
      <th scope="col">Presription ID</th>
      <th scope="col">Date</th>
      <th scope="col">Time</th>
      <th scope="col">Description</th>
      <th scope="col">Prescription medicines</th>
  ...
  <tbody>
    ...
    if(isset($_POST['submit'])){
      ...
      if($date == ""){
        echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
          <strong>WARNING!</strong> MUST ENTER DATE
          <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>';
      }else{
      $sql = "SELECT prescription.Press_id, prescription.serial, date, time, description, Pres_medicine from (((takes inner join services on takes.service_serial = services.serial) inner join prescription on takes.service_serial = prescription.Serial) inner join prescription_medicine on prescription.Press_id = prescription_medicine.press_id) where takes.p_id = '$patient_id' and date = '$date' order by date desc, time desc";
      //getting all info
    ...
    $sql = "SELECT prescription.Press_id, prescription.serial, date, time, description, Pres_medicine from (((takes inner join services on takes.service_serial = services.serial) inner join prescription on takes.service_serial = prescription.Serial) inner join prescription_medicine on prescription.Press_id = prescription_medicine.press_id) where takes.p_id = '$patient_id' order by date desc, time desc";
    //getting all info
    $result = mysqli_query($conn, $sql); 
    $num = mysqli_num_rows($result);
